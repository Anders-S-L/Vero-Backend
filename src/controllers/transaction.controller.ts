...
export const createTransactionController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const data = await createTransaction(req.userProfile!, amount, date, category_id, description, repeat_monthly, repeat_until)
...
export const updateTransactionController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const data = await updateTransaction(req.userProfile!, id, amount, date, description || null)
        res.status(200).json({ success: true, data })
    } catch (error) {
        res.status(400).json({ success: false, error: (error as Error).message })
    }
